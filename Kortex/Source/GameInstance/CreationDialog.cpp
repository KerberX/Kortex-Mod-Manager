...
namespace Kortex::GameInstance
{
	...
	bool CreationDialog::Create(wxWindow* parent, const GameID& gameID)
	{
		...
		if (KxComboBoxDialog::Create(parent, wxID_NONE, KTrf("InstanceCreatorDialog.Caption", m_InstanceTemplate->GetGameName()), wxDefaultPosition, wxDefaultSize, KxBTN_OK|KxBTN_CANCEL, DefaultStyle|KxCBD_READONLY))
		{
			...
			KxLabel* nameLabel = new KxLabel(contentPanel, wxID_NONE, KTr("InstanceCreatorDialog.NameLabel"));
			nameSizer->Add(nameLabel, 1);

			m_NameInput = new KxTextBox(contentPanel, wxID_NONE);
			nameSizer->Add(m_NameInput, 1);

			// Copy from
			wxBoxSizer* copySizer = new wxBoxSizer(wxHORIZONTAL);
			mainSizer->Add(copySizer, 0, wxEXPAND|wxTOP, LayoutConstants::VerticalSpacingSmall);

			KxLabel* copyFromLabel = new KxLabel(contentPanel, wxID_NONE, KTr("InstanceCreatorDialog.CopyFrom"));
			copySizer->Add(copyFromLabel, 1, wxTOP, LayoutConstants::VerticalSpacingSmall);
			...
			copyOptionsSizer->Add(m_InstancesList, 0, wxEXPAND|wxTOP, LayoutConstants::VerticalSpacingSmall);
			...
			m_CopyInstanceConfigCHK = new KxCheckBox(contentPanel, wxID_NONE, KTr("InstanceCreatorDialog.CopyInstanceConfig"));
			copyOptionsSizer->Add(m_CopyInstanceConfigCHK, 0, wxEXPAND|wxTOP, LayoutConstants::VerticalSpacingSmall);
	...
	{
		if (event.GetId() == wxID_OK && !OnOK(event))
		{
			event.Veto();
		}
	}
