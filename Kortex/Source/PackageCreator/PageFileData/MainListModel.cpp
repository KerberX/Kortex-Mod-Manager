...
namespace Kortex::PackageDesigner::PageFileDataNS
{
	...
	void MainListModel::GetValueByRow(wxAny& value, size_t row, const KxDataViewColumn* column) const
	{
		if (const PackageProject::FileItem* item = GetDataEntry(row))
		{
			switch (column->GetID())
			{
				case ColumnID::ID:
				{
					value = KxDataViewBitmapTextValue(item->GetID(), ImageProvider::GetBitmap(item->QueryInterface<PackageProject::FolderItem>() ? Imagekxf::ResourceID::Folder : Imagekxf::ResourceID::Document));
					break;
				}
				case ColumnID::Source:
				{
	...
	void MainListModel::OnContextMenu(KxDataViewEvent& event)
	{
	...
	
		{
			...
			item->SetBitmap(ImageProvider::GetBitmap(Imagekxf::ResourceID::DocumentPlus));
		}
		{
